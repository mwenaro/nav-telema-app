...
import LogoutOptionModal from "./LogoutOptionModal";

export default function DashboardAuthNav({
  className = "",
  vendor = "",
}: {
  className?: string;
  ...
  return (
    <div
      className={twMerge(
        `w-3/12  min-w-[200px] grid grid-cols-2 content-center`,
        className
      )}
    >
      ...
      <div className="grid grid-cols-2">
        ...
        <div className="flex-1 p-3  border-l-2 border-slate-500 flex items-center justify-center relative">
          ...
          />
          {/* Logout Modal */}
          {isModalOpen && <LogoutOptionModal vendor={vendor} />}
        </div>
      </div>
    </div>
  );
